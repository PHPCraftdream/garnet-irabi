...
Node-зависимости сборки объявлены в `FrontBuilder/package.json`, но
резолвятся от файлов в `Bundle/Front/`: и бандлер, и PostCSS ищут модули,
поднимаясь вверх от импортирующего файла, а этот путь до `FrontBuilder/` не
доходит. Поэтому в корне пакета фреймворка лежит ссылка `node_modules` →
`FrontBuilder/node_modules`.

Composer при обновлении пакета удаляет его каталог целиком — вместе со
ссылкой и установленными модулями. Раньше сборка после этого падала с
`Can't resolve 'tailwindcss'`, сообщением, которое не называет ни причины,
ни лечения.

Начиная с `v0.1.0-alpha34` это чинит сам `php garnet prepare`: ставит
зависимости (`npm ci`, если каталог пуст или отсутствует) и восстанавливает
ссылку. Отдельных действий после `composer update` не требуется — достаточно
обычной последовательности `prepare` → `build`.
