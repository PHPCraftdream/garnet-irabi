...
**Таблица:** `ir_accounts_data`
**Поле:** `IS_APPROVED` (EAV-параметр аккаунта)

> До `M_0023` рядом жила копия этого флага в `ir_expert_profiles`. Копия
> отставала: на боевом она осталась нулевой у троих одобренных
> преподавателей, и экраны, спрашивавшие её, показывали пустоту. Таблица
> удалена, состояние читается только отсюда.
