...
- **Статус: ОТКАЧЕНО в report 14** (изначально было ИСПРАВЛЕНО с self/staff/counterparty-гейтом). При аудите report 14 (F-14-02) обнаружено, что `/users/~preview` (`UsersController::post__preview`) отдаёт те же самые агрегированные booking/cancellation-счётчики любого обычного пользователя любому залогиненному — то есть гейт M-03 на `/user/id~N` был обходим через другой эндпоинт, и две поверхности профиля противоречили друг другу. Владелец продукта принял решение: **эти счётчики публичны везде** (email/login по-прежнему никогда не раскрываются). Поэтому гейт `canViewProfile()` удалён, `/user/id~N` снова публичен для любого authenticated actor, добавлена только анонимизация disabled-аккаунтов (`AccountDisplay`, единообразно с остальными surface). Тест `Tests/cross-role/user-profile-access-policy.spec.ts` переписан под новую публичную политику + проверку анонимизации disabled. Подробности — в `14-ms-postfix-authorization-review.md`.
- **Severity:** Medium
