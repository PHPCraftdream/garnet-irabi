...
Ограничение: предыдущие отчеты `docs/audits/security/*` не использовались как источник анализа.
...
- `rg --files Apps\IRabi -g '!docs/audits/security/**' -g '!vendor/**'`
...
- `git status --short -- Apps/IRabi/docs/audits/security/09-ms-fresh-authorization-review.md` failed because `D:\dev\garnet` is not a git repository root for that command context.
