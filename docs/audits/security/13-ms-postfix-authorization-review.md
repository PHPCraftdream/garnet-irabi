...
2. `MEDIUM` → **не баг, accepted policy.** owner/admin-only для manual balance adjustment — намеренное решение (`DashboardFinanceController.php:341-342`: "Money movement is owner/admin-only... see security audit H-1"), и `docs/guides/product/roles.md:53-56` явно относит финансовый контроль/выплаты к роли Owner ("полное управление бизнесом"), тогда как Admin в том же документе (`roles.md:17-36`) описан только как техническая роль без бизнес-доступа. Отчёт не нашёл этот документ, отсюда ложное противоречие.
...
**Invariant:** manual ledger entry типа `manual` должен создаваться только тем rank, который утвержден финансовой политикой. В `docs/uat/state-machine/admin.md` ручная корректировка описана как admin-only; код и inline comment говорят owner/admin-only.
