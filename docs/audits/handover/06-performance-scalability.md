...
- **P3-1. Мёртвая сборка `Public/assets/app-irabi/` (~3 МБ, 56 файлов)** — на неё нет ни одной ссылки в PHP (единственное упоминание `app-irabi` — имя директории кода в `docs/guides/operations/deploy.md:31`); это артефакт старого `public_name`. Лежит в docroot, попадает в деплой и бэкапы. Удалить из поставки.
