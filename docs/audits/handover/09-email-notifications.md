...
- Рекомендация: добавить `->text(strip_tags(...))` в Mailer, раздел «Почта: SPF/DKIM/DMARC,
  From-домен» в `docs/guides/operations/deploy.md`, и в идеале `List-Unsubscribe: <mailto:...>,<https://.../~profile>`.
...
3. **Утечка секретов в теле писем** (п.3в). Код подтверждения в auth-письме — ожидаемо
   ...
   ввода в заголовки (подтверждает и предыдущий аудит `docs/audits/security/04`).
