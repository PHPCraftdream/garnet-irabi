/**
 * Подготовка для проверок добора возврата: сеяние уже отменённой брони,
 * подсчёт проводок, вход под роль.
 *
 * Сеется напрямую в базу — иначе состояние «бронь отменена, а возврата
 * нет» не создать: контроллер его не допускает, а проверять надо именно
 * его, потому что в жизни оно получалось при обрыве на середине.
 */
 ...
 */
export async function seedCancelledBooking(params: {
	userId: number;
	expertId: number;
	slotId: number;
	cost: number;
...
export async function countRefunds(bookingId: number): Promise<{ userCredit: number; expertDebit: number }> {
	const conn = await mysql.createConnection(DB);
	try {
		const [rows] = await conn.execute<any[]>(
