...
test.describe('Cross-role: booking + expert cancels + refund', () => {
	...
	test.beforeAll(async ({ browser }) => {
		...
		// Сначала свести кэш с журналом, потом снимать базу.
		//
		// `getBalance()` читает КЭШ (`account_balance.balance`), а источник
		// истины — журнал: продукт зачисляет эксперту строку
		// `booking_payment` и вызывает `AccountBalance::recalculate()`,
		// который переписывает кэш СУММОЙ ПО ЖУРНАЛУ, а не «кэш + cost».
		// Поэтому утверждение `expertBalanceBefore + SLOT_COST` выполнимо
		// только если на момент снятия базы кэш журналу равен. Соседние
		// проверки регулярно оставляют его расхождённым (чистят строки
		// журнала по брони, правят кэш отдельным UPDATE) — и тогда
		// ожидаемого числа продукт не выдаст никогда: `expect.poll`
		// выжигает все 10 секунд и падает, а повтор проходит, потому что
		// упавший прогон своим же recalculate() кэш и выровнял. Так этот
		// шаг и флейкал во всех семи прод-прогонах (D-220).
		//
		// Тот же приём и с тем же обоснованием уже стоит в
		// `cross-role/booking/booking-penalty-cancel.spec.ts`.
	...
	test('step 3: expert balance increased by SLOT_COST', async () => {
		...
		// Ждём значение, а не читаем один раз.
		//
		// Оговорка к прежнему комментарию здесь: зачисление НЕ приходит
		// «после ответа». `SlotsController::post__book` вставляет строку
		// `booking_payment` и вызывает `recalculate($expertId)` в своём
		// finally — всё внутри того же запроса, до ответа. Так что к
		// моменту этой проверки запись давно на месте, и ожидание тут не
		// от медленной записи, а страховка на случай, когда кэш эксперту
		// перепишет что-то ещё. Настоящая причина флейка — расхождение
		// кэша с журналом на момент снятия базы, и лечится оно в
		// beforeAll (см. комментарий там), а не длиной ожидания.
		await expect.poll(
