...
const ExpertAvatar: React.FC<{expert: Expert; initials: string; onOpenLightbox: () => void}> = ({expert, initials, onOpenLightbox}) => {
    if (expert.is_disabled) {
        return (
            <div
...
const ExpertActions: React.FC<{expertId: number; isOwnProfile: boolean}> = ({expertId, isOwnProfile}) => (
...
const ExpertStatCell: React.FC<{testId: string; className: string; value: number; label: string}> = ({testId, className, value, label}) => (
    <div className="profile-stat-cell">
        <div className={className} data-test-id={testId}>{value}</div>
...
const ExpertStats: React.FC<{expert: Expert}> = ({expert}) => (
    <div className="profile-card mb-5" data-test-id="expert-stats">
        <div className="grid grid-cols-2 md:grid-cols-5">
            <ExpertStatCell testId="expert-stat-conducted" className="profile-stat-value text-success" value={expert.conducted_count ?? 0} label={t.Expert_Conducted()} />
            <ExpertStatCell testId="expert-stat-upcoming" className="profile-stat-value text-accent" value={expert.upcoming_count ?? 0} label={t.Expert_Upcoming()} />
            <ExpertStatCell testId="expert-stat-declines" className="profile-stat-value text-warning" value={expert.decline_count ?? 0} label={t.Expert_Declines()} />
            <ExpertStatCell testId="expert-stat-cancellations" className="profile-stat-value text-warning" value={expert.cancellation_count ?? 0} label={t.Expert_Cancellations()} />
            ...
            <ExpertStatCell testId="expert-stat-missed" className="profile-stat-value text-muted" value={expert.missed_count ?? 0} label={t.Expert_Missed()} />
        </div>
...
const SlotsShowMore: React.FC<{hasMore: boolean; remaining: number; onShowMore: () => void}> = ({hasMore, remaining, onShowMore}) => (
    <div className="mt-4 text-center">
        {hasMore ? (
            <button
                ...
                onClick={onShowMore}
            >
                {t.Expert_ShowMoreSlots()} ({remaining})
            </button>
        ) : (
    ...
    return (
        <div className="page-narrow" data-test-id="expert-profile">
            ...
            <div className="flex items-start gap-4 mb-6">
                <ExpertAvatar expert={expert} initials={initials} onOpenLightbox={() => setLightboxOpen(true)} />
                <div className="flex flex-col justify-center min-w-0">
                    <h1 className="mb-1 text-on-surface">{expert.display_name}</h1>
                    <ExpertActions expertId={expertId} isOwnProfile={isOwnProfile} />
                </div>
            </div>
            ...
            {slots.length > SLOTS_PAGE_SIZE && (
                <SlotsShowMore
                    hasMore={hasMore}
                    remaining={slots.length - visibleCount}
                    onShowMore={() => setVisibleCount(c => c + SLOTS_PAGE_SIZE)}
                />
            )}
